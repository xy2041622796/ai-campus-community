...
### 设计理念转变
从"系统用 AI"转变为"用户用 AI"：
- **之前**：AI 藏在按钮后面（润色、审核、标签）
- **现在**：AI 主动出现在用户需要的地方

### 核心原则
1. **降低创作成本**：AI 帮用户发帖、评论、组织活动
2. **提高互动效率**：AI 帮用户找人、找活动、找内容
3. **增强陪伴感**：AI 在浏览、互动的过程中主动提供帮助

### 实现的功能矩阵

| 功能 | 入口 | AI 能力 | 用户感知 |
|------|------|---------|---------|
| AI 发帖助手 | 发帖页顶部 | 一句话生成完整帖子 | 真方便 |
| AI 评论助手 | 评论区 | 3条一键回复 | 点一下就发 |
| AI 首页个性化 | 首页 Hero | 时间问候+洞察卡片 | 社区在关心我 |
| AI 找搭子 | 发现页 | 自然语言匹配推荐 | 找人好快 |
| AI 活动助手 | 活动创建页 | 一句话生成活动方案 | 组织活动好简单 |
| AI 社区助手 | 全局浮动按钮 | 自然语言搜索社区 | 整个社区都能搜 |

### 技术实现要点
- 所有 AI 功能统一使用 Agnes AI (agnes-2.0-flash)
- 返回 JSON 格式，前端解析后自动填充表单
- 浮动助手组件独立于页面，全局可用
- 搜索逻辑基于现有 store 数据（post/event/recommend）

### 产品价值
- 用户感受到的是"整个社区都在智能地帮助我"
- 而不是"这里有很多 AI 功能"
- AI 从工具变成了助手
